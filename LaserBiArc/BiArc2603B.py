...
# print .. dont use when running as extension in inkscape
...
class CubicBezier:
    ...
    def inflectionPoints(self):
        ...
        ang=math.atan2(yt4,xt4)

        debug1(dn,1,"xt4=%s yt4=%s ang=%s" % (xt4,yt4,ang))    
        
        # and rotate so y4 becomes 0
        x2 = xt2*math.cos(-ang) - yt2*math.sin(-ang) 
        x3 = xt3*math.cos(-ang) - yt3*math.sin(-ang) 
        x4 = xt4*math.cos(-ang) - yt4*math.sin(-ang)         
        y2 = xt2*math.sin(-ang) + yt2*math.cos(-ang) 
        y3 = xt3*math.sin(-ang) + yt3*math.cos(-ang) 
...
def bezier2biarcs(mbezier, resolution):
    ...
    #        isOnLine(fromPoints(mbezier._c1, mbezier._c2), mbezier._p1)):
    if mbezier._p1 == mbezier._p2 and not isOnLine(fromPoints(mbezier._c1, mbezier._c2), mbezier._p1): #JK 2026.01.31
        #(isOnLine(fromPoints(mbezier._c1, mbezier._c2), mbezier._p1) and isOnLine(fromPoints(mbezier._c1, mbezier._c2), mbezier._p2)): #JK 2026.01.31
    ...
    elif ( #isOnLine(fromPoints(mbezier._p2, mbezier._p1), mbezier._c1) #and isOnLine(fromPoints(mbezier._p2, mbezier._p1), mbezier._c2):
            isOnLine(fromPoints(mbezier._p1, mbezier._p2), mbezier._c1) 
            and isOnLine(fromPoints(mbezier._p1, mbezier._p2), mbezier._c2) ): #JK 2026.01.31

        debug1(dn,9,"All points on line..LineTo..p1=%s,%s p2=%s,%s" % (mbezier._p1[0],mbezier._p1[1],mbezier._p2[0],mbezier._p2[1]))
    ...
    elif distance(mbezier._c1,mbezier._c2) < 1*resolution: #test 6*
        return approxSpiral(mbezier, resolution)


    else:
        debug1(dn,9,"byInflection") 
...
def approxSpiral(bezier, resolution):
    ...
    # Check for Closed or Almost Closedcurve first
    #if bezier.maxArcLength() < resolution and (bezier._p1 != bezier._c1 and bezier._p2 != bezier._c2):
    if ( bezier._p1 == bezier._p2 or distance(bezier._p1, bezier._p2) < resolution 
            and not isOnLine(fromPoints(bezier._c1, bezier._c2), bezier._p1) 
            and not isOnLine(fromPoints(bezier._c1, bezier._c2), bezier._p2) ):     #JK 2026.01.31
        debug1(dn,8,"1 Closed or Almost closed")
        debug1(dn,9,"1 Closed or Almost closed")
    ...
    if bezier.maxArcLength() < resolution:
        ...
        debug1(dn,9,"2 aproxSpiral maxArcLength")
        ...
        c1 = bezier._c1
        c2 = bezier._c2
        #if distance p1,c1 < resolution ..degenerate curve .. use c1=c2
        if distance(bezier._p1, bezier._c1) < resolution:
             c1 = bezier._c2
        #if distance p2,c2 < resolution ..degenerate curve .. use c2=c1
        if distance(bezier._p2, bezier._c2) < resolution:
             c2 = bezier._c1
