...
from BiArc2603 import CubicBezier, BiArc, bezier2biarcs, LineTo, ArcTo, distance, BiArc_par    #, isClockwise3 not used now
            ...
            for np,path in enumerate(path_list):
                ...
                for npc,csp in enumerate(csp_list):                            #cp (control point) or pt (point) instead of pt ??
                    ...
                    for nc in range(0,len(csp)-1):      # nc and nc+1 is used as index (n) in csp (cubic superpath)  (ncsp !! bättre)                       
                        ...
                        #self.msg(bez)
                        cmds = bezier2biarcs(bez, resolution)      # was 0.1 hard coded .. now declared at top of file
                        #self.msg(cmds)
