# BiArc2602.py
# def inflectionPoints(self): .. added code for inflectionPoints .. JK 2025.12.29
# def BA_fromPoints .. cw1= .. now > 0, was < 0 JK 2026.01.21    
# def bezier2biarcs .. changes
# def approxSpiral .. changes

# License: MIT
# Copyright (c) Jan Knuts EM-Teknik (jan.knuts@gmail.com)
# https://github.com/EM-Teknik/LaserBiArc
#
# BiArc.py (original file)  is a translation from haskell code BiArc.hs
...
# Global constants
# ------------------------
eps = 0.0001    #original 0.0001
maxiter = 10    #original 10
...
class CubicBezier:
    ...
    def inflectionPoints(self):
        ...
        return []
    ''' 
    # added code for that.. JK 2025.12.29
    def inflectionPoints(self):
        #
        # from: https://pomax.github.io/bezierinfo/#inflections
        #
        # notation: x1,x2,x3,x4 corresponds to: p1,c1,c2,p2

        dn=BiArc_par.dn  #for debug1
        use_inflP=BiArc_par.use_inflP

        if not use_inflP:
            debug1(dn,1,"dont use inflP")    
            return []
        else:
            debug1(dn,1,"use inflP")    
        #return []
        # first translate so x1=[0,0]    
        xt2 = self._c1[0]-self._p1[0]
        xt3 = self._c2[0]-self._p1[0]
        xt4 = self._p2[0]-self._p1[0]
        yt2 = self._c1[1]-self._p1[1]
        yt3 = self._c2[1]-self._p1[1]
        
        # calc angle to xt4,yt4
        # need yt4
        yt4 = self._p2[1]-self._p1[1]
        v=math.atan2(yt4,xt4)

        debug1(dn,1,"xt4=%s yt4=%s v=%s" % (xt4,yt4,v))    
        
        # and rotate so y4 becomes 0
        x2 = xt2*math.cos(-v) - yt2*math.sin(-v) 
        x3 = xt3*math.cos(-v) - yt3*math.sin(-v) 
        x4 = xt4*math.cos(-v) - yt4*math.sin(-v)         
        y2 = xt2*math.sin(-v) + yt2*math.cos(-v) 
        y3 = xt3*math.sin(-v) + yt3*math.cos(-v) 
        
        # calculation needs the above translate and rotate
        ...
        y = (3*a - b - 3*c)*18
        z = (c - a)*18
        if x != 0 and (y*y-4*x*z) > 0:
            t1 = (-y + math.sqrt(y*y-4*x*z))/(2*x)
            t2 = (-y - math.sqrt(y*y-4*x*z))/(2*x)
        else:
            return[]

        #
        # end from: https://pomax.github.io/bezierinfo/#inflections
        #
        ...
        debug1(dn,1,"t1=%s ,t2=%s" % (t1,t2))
        
        #if (BiArc_par.dn == 1):
        #    debug("t1=%s ,t2=%s" % (t1,t2))
        
        if (t1_ok  and not t2_ok):
            debug1(dn,1,"t1=%s" % (t1))

            return [t1]
        elif (t2_ok and not t1_ok):
            debug1(dn,1,"t2=%s" % (t2))
            ...
            if (t1 < t2):
                ...
                debug1(dn,1,"t2=%s ,t2=%s" % (t2,t1))

                return [t2,t1]
        else:
            return []
    # end added code inflectionPoints
...
class BiArc_par:
    def __init__(self, use_inflP, dn):
        self.use_inflP = use_inflP
        self.dn = dn
    '''
    def set_use_inflP(s_inflP):
        self.use_inflP=s_inflP    
    def use_inflP():
        return self.use_inflP
    def dn():
        return self.dn
    '''
'''
def debugT1(self,dn,no,msg):
    if no==dn:
        debug(msg)
'''    
...
def BA_fromPoints(p1, d1, p2, d2, g):
    ...
    v1 = sub(p1, center1)
    v2 = sub(g, p1)
    cw1 = (v1[0]*v2[1] - v1[1]*v2[0]) > 0 # def BA_fromPoints .. cw1 = .. now > 0, was < 0 JK 2026.01.21 
...
def bezier2biarcs(mbezier, resolution):
    dn=BiArc_par.dn  #for debug1
    # Closed curve JK
    #if (mbezier._p1 != mbezier._c1 or mbezier._p2 != mbezier._c2) and mbezier._p1 == mbezier._p2 : #JK 2026.01.31
    if mbezier._p1 == mbezier._p2 : #JK 2026.01.31
        debug1(dn,9,"Closed curve..p1=%s,%s p2=%s,%s" % (mbezier._p1[0],mbezier._p1[1],mbezier._p2[0],mbezier._p2[1]))
        return approxSpiral(mbezier, resolution) 
    # Degenerate curve: all points on the same line -> it is a line 
    elif (isOnLine(fromPoints(mbezier._p2, mbezier._p1), mbezier._c1) and
        isOnLine(fromPoints(mbezier._p2, mbezier._p1), mbezier._c2)):
        debug1(dn,9,"All points on line..p1=%s,%s p2=%s,%s" % (mbezier._p1[0],mbezier._p1[1],mbezier._p2[0],mbezier._p2[1]))
        return [LineTo(toPoint(mbezier._p2))]
    # Degenerate curve: p1 == c1, don't split
    elif mbezier._p1 == mbezier._c1:
        debug1(dn,9,"bezier2biarcs.._p1==_c1") 
        #return [LineTo(toPoint(mbezier._p2))] #Test
        return approxSpiral(mbezier, resolution)
...
def byInflection(mbezier, ts, resolution):
    ...
    #print("byInflection..ts=",ts) 
    #ts0_ok= (ts[0]>resolution and ts[0]< 1-resolution)  
    if len(ts) == 1 and ts[0]>resolution and ts[0]< 1-resolution:    #JK 2026.01.26
        #print("byInflection..len(ts) == 1")    
        (b1, b2) = mbezier.splitAt(ts[0])
        return approxSpiral(b1, resolution) + approxSpiral(b2, resolution)
    elif len(ts) == 2:
        #print("byInflection..len(ts) == 2")
...
def approxSpiral(bezier, resolution):
    ...
    '''
    debug1(dn,8,"0 aproxSpiral p1=%s,%s c1=%s,%s c2=%s,%s p2=%s,%s" % (bezier._p1[0],bezier._p1[1],bezier._c1[0],bezier._c1[1],bezier._c2[0],bezier._c2[1],bezier._p2[0],bezier._p2[1]))
    # Check for closed curve first  JK
    # Edge case: start- and endpoints are the same
    if bezier._p1 == bezier._p2:
    #if distance(bezier._p1, bezier._p2) < eps: #JK funkar ej...
        debug1(dn,8,"1 aproxSpiral _p1==_p2")
        return splitAndRecur(bezier, 0.5, resolution)
    # Then check distance between P1,P2 , will be zero for closed curve  JK
    # Approximate bezier length. if max length is smaller than resolution, do not approximate
    elif bezier.maxArcLength() < resolution:
        debug1(dn,8,"2 aproxSpiral maxArcLength")
        return [LineTo(toPoint(bezier._p2))]

        '''
    # Degenerate curve , 2 cases
    elif bezier._p1 == bezier._c1:  # Degenerate curve at beginning , split near degenerated end
        debug1(dn,1,"_p1==_c1")
        debug1(dn,9,"3 aproxSpiral _p1==_c1 (%s)" % (bezier._p1 == bezier._c1))

        (b1, b2) = bezier.splitAt(0.1) #test
        debug1(dn,9,"3 aproxSpiral b1_p1=%s,%s b1_c1=%s,%s b1_c2=%s,%s b1_p2=%s,%s" % (b1._p1[0],b1._p1[1],b1._c1[0],b1._c1[1],b1._c2[0],b1._c2[1],b1._p2[0],b1._p2[1]))
        debug1(dn,9,"3 aproxSpiral b2_p1=%s,%s b2_c1=%s,%s b2_c2=%s,%s b2_p2=%s,%s" % (b2._p1[0],b2._p1[1],b2._c1[0],b2._c1[1],b2._c2[0],b2._c2[1],b2._p2[0],b2._p2[1]))

        return splitAndRecur(bezier, 0.1, resolution)
    elif bezier._p2 == bezier._c2:  # Degenerate curve at end , split near degenerated end
        debug1(dn,1,"_p2==_c2")
        debug1(dn,9,"4 aproxSpiral _p2==_c2")
        return splitAndRecur(bezier, 0.9, resolution)
    #tangents parallel
    elif  sub(bezier._c1,bezier._p1) == sub(bezier._c2,bezier._p2):
 
        return splitAndRecur(bezier, 0.5, resolution)
        '''

    else:
        debug1(dn,8,"5 aproxSpiral else")
        debug1(dn,8,"5 parallel.. %s" % (sub(bezier._c1,bezier._p1) == sub(bezier._c2,bezier._p2)))
        ...
        # Test for degenerate curves JK
        c1 = bezier._c1
        c2 = bezier._c2
        #if distance p1,c1 < eps ..degenerate curve .. use c1=c2
        if distance(bezier._p1, bezier._c1) < eps:
             c1 = bezier._c2
        #if distance p2,c2 < eps ..degenerate curve .. use c2=c1
        if distance(bezier._p2, bezier._c2) < eps:
             c2 = bezier._c1
        ...
        debug1(dn,88,"5 t1=%s,%s t2=%s,%s" % (t1.p1,t1.p2,t2.p1,t2.p2))
        debug1(dn,88,"5 m1=%s m2= %s" % (m1,m2))
        debug1(dn,98,"5 math.isnan(m1) .. %s" % (math.isnan(m1)))
        debug1(dn,98,"5 math.isnan(m2) .. %s" % (math.isnan(m2)))

        #if (m1 == m2) or (math.isnan(m1) and math.isnan(m2)): #JK
        if (abs(m1 - m2)< eps) or (math.isnan(m1) and math.isnan(m2)): #JK 
            return splitAndRecur(bezier, 0.5, resolution) #JK 0.1
            '''            
            if math.isnan(m1):  
                return splitAndRecur(bezier, 0.5, resolution) #JK 0.1
            elif math.isnan(m2):
                return splitAndRecur(bezier, 0.5, resolution) #JK 0.9
            else:
                return splitAndRecur(bezier, 0.5, resolution)
            '''
        # V: Intersection point of tangent lines
        v = intersection(t1, t2)
        debug1(dn,8,"5 v=%s,%s" % (v[0],v[1]))
        #'''
        if math.isnan(v[0]) and math.isnan(v[1]):
            debug1(dn,8,"5 c1=%s,%s c2=%s,%s" % (c1[0],c1[1],c2[0],c2[1]))
            debug1(dn,8,"5 t1=%s,%s t2=%s,%s" % (t1.p1,t1.p2,t2.p1,t2.p2))
            return [LineTo(toPoint(bezier._p2))] #JK 
        #'''         
        # Biarc triangle has the wrong orientation
        if (bezier.isClockwise() != isClockwise3(bezier._p1, bezier._p2, v)): #JK var !=
            debug1(dn,8,"5 Biarc triangle has the wrong orientation")
            #return [LineTo(toPoint(bezier._p2))] #JK
            return splitAndRecur(bezier, 0.5, resolution)

        # Compute distances for incenter computation
        debug1(dn,8,"5 Compute distances for incenter computation")
        dP2V = distance(bezier._p2, v)
        dP1V = distance(bezier._p1, v)
        dP1P2 = distance(bezier._p1, bezier._p2)
        sum_d = dP2V + dP1V + dP1P2
        if abs(sum_d) < eps: #JK test 10*eps
            g = (0,0)
        else:
        ...
        if not isStable(biarc):
            debug1(dn,8,"5 not isStable")
            return splitAndRecur(bezier, 0.5, resolution)
        elif maxDistance > resolution:
            debug1(dn,8,"5 maxDistance > resolution")
            return splitAndRecur(bezier, maxDistanceAt, resolution)
        else:
            debug1(dn,8,"5 return biarc2path")           
...
if __name__ == '__main__':
    ...
    #[[[5.46,35.53],[5.74,33.03],[5.46,15.53],[5.46,5.53]]];  #test Jan
    bez = CubicBezier((5.46,35.53), (5.74,33.03), (5.46,15.53), (5.46,5.53))
    
    cmds = bezier2biarcs(bez, 0.1)   #orginal 0.1
    for cmd in cmds:
        if isinstance(cmd, LineTo):
            print("LineTo", cmd.point)
        elif isinstance(cmd, ArcTo):
